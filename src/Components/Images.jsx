import React, { useState } from 'react'



export default function Images() {


const [classAdi,setClassAdi] = useState("")  //varsayılan değer(initial)

//useState te getterlar ve setterlar vardır
//getterlar veriyi tutmakla yükğmlüdür
//setterlar ise veriyi güncellemekle yükümlüdür


const changeView = () => {
    if(classAdi === "flex-container "){
        setClassAdi("flex-container active")
    }else{
        setClassAdi("flex-container ")
    }
}


  return (



    <>
    
    
        <div>
            <button onClick={changeView}>Görünümü Değiştir</button>
        </div>

    <div className={classAdi}>

        <div>
            <img src="data:image/jpeg;base64," alt="" />
        </div>

        <div>
            <img src="data:image/jpeg;base64," alt="" />
        </div>

        <div>
            <img src="data:image/jpeg;base64," alt="" />
        </div>

    </div>
    
    
    </>

    


  )
}
